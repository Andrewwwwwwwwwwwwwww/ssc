...
The corpse hitbox is an ordinary `minecraft:interaction` entity, so claim mods treat looting a body as "interacting with an entity" and block it inside claims. SSC ships an entity tag for this — add it to OPAC's forced exceptions in `<world>/serverconfig/openpartiesandclaims-server.toml` (the server must be stopped to edit it):

```toml
forcedEntityProtectionExceptionList = ["minecraft:minecart", "anything$#ssc:corpses"]
```

The `anything# Server Sided Corpse (SSC)
...
| `skeletonStageIsPublic` | `true` | Aged corpses may be looted by anyone. |
| `deathHistorySize` | `20` | Deaths kept per player for `/deathhistory`. |
 prefix matters: without it the exception only applies when the item in your hand isn't itself blocked, so a player holding a sword still couldn't loot their body. SSC does its own owner check regardless of claims, so a body stays locked to its owner either way.

Note that `#ssc:corpses` resolves to `minecraft:interaction`, the vanilla type SSC uses for hitboxes — whitelisting it exempts every interaction entity on the server, not only corpses. That is only a concern if another mod or datapack on your server also uses interaction entities for something it wants protected.

SSC prints this config line to the console at startup whenever it detects OPAC.

## Diagnosing a missing body

Bodies are drawn with packets rather than being real entities, so `/data` and the F3 entity list can't see them. Use:

| Command | What it does |
| --- | --- |
| `/ssc list` | Every body on the server: owner, position, dimension, items, XP, age, and **how many players can currently see it**. |
| `/ssc resend [player]` | Forget what a player's client has been sent, so every body in range is re-sent within two seconds. |
| `/ssc debug <true\|false>` | Log every decision the death handler makes — including each reason it declines to create a body. |

If a body never appears for anyone, turn on `/ssc debug true` and check the console on the next death: every path that skips a body says why. If a body appears for everyone *except* one player, that client lost the packets — `/ssc resend <player>` fixes it immediately, and the automatic resync should prevent it recurring.
