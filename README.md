...
The `anything$` prefix matters: without it the exception only applies when the item in your hand isn't itself blocked, so a player holding a sword still couldn't loot their body. SSC does its own owner check regardless of claims, so a body stays locked to its owner either way.
